...
suite('Extension Manifest Test Suite', () => {
	...
	const commandIds = (manifest.contributes?.commands ?? []).map((c: { command: string }) => c.command);
	const activationEvents: string[] = manifest.activationEvents ?? [];

	test('S26: resetSizes.openActivityLog is declared (one of three log entry points)', () => {
		assert.ok(
			commandIds.includes('resetSizes.openActivityLog'),
			'Activity log Command Palette entry must be contributed in the manifest'
		);
	});

	test('S34: resetSizes.resetAll is declared', () => {
		assert.ok(
			commandIds.includes('resetSizes.resetAll'),
			'Reset command must be contributed so users can bind their own shortcut'
		);
	});

	test('S34: manifest does NOT contribute a default keybinding', () => {
	...
	test('S35: manifest contributes no "preset" enum with a "custom" value', () => {
		...
		assert.ok(!('resetSizes.commands' in props), 'resetSizes.commands must not appear in the manifest');
		assert.ok(!('resetSizes.settingsToReset' in props), 'resetSizes.settingsToReset must not appear in the manifest');
		assert.ok(!('resetSizes.scopes' in props), 'resetSizes.scopes must not appear in the manifest');
		assert.ok(!('resetSizes.reloadAfter' in props), 'resetSizes.reloadAfter must not appear in the manifest');
	});

	test('Manifest contributes resetSizes.confirmBeforeDestructiveReset (default true)', () => {
		assert.ok('resetSizes.confirmBeforeDestructiveReset' in props, 'New preference must be declared');
		assert.strictEqual(
	...
	test('S19: manifest contributes resetSizes.reloadSilently (default false, reversible from settings)', () => {
		assert.ok('resetSizes.reloadSilently' in props,
			'S19: silent-reload preference must be declared as a settings-page toggle');
		assert.strictEqual(props['resetSizes.reloadSilently'].type, 'boolean',
			'reloadSilently must be a boolean');
		assert.strictEqual(props['resetSizes.reloadSilently'].default, false,
	...
	test('S26: showSummaryNotification description carries a command link to the activity log', () => {
		const prop = props['resetSizes.showSummaryNotification'];
	...
	test('S27: resetSizes.openPreview is declared (Command Palette entry point)', () => {
		assert.ok(
			commandIds.includes('resetSizes.openPreview'),
	...
	test('Manifest activation events include the new preview command', () => {
		assert.ok(
			activationEvents.includes('onCommand:resetSizes.openPreview'),
